# Proyecto ODS 4 – Educación de Calidad

---

## Día 1

| Tarea | Quién | Estado |
|-------|-------|--------|
| Crear el repositorio en GitHub | Iván |  Terminado |
| Organizar las carpetas del proyecto | Alberto |  Terminado |
| Hacer el formulario HTML con todos los campos | Alejandro |  Terminado |

---

## Día 2

| Tarea | Quién | Estado |
|-------|-------|--------|
| Darle estilo con CSS | Iván |  Terminado |
| Programar el crear libro en JavaScript | Alberto |  Terminado |
| Programar el listar libros en JavaScript | Alejandro |  Terminado |
| Hacer el README del proyecto | Iván |  En proceso |


## Día 3 
| Tarea | Quién | Estado |
|-------|-------|--------|
| cambios en el css | Iván y Alberto | Terminado |
| crear vistas de formulario e historial | Ivan y Alejandro | Terminado |
| terminar el crud | Alejandro |  Terminado |
| Avanzar el README del proyecto | Alberto |  Terminado |

